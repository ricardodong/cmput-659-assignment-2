...
class Game:
    ...
    def count_neutral_markers(self):
        ...
        # 0 represent the marker of current player
        # return the number used for current player
        count = 0
    ...
    def is_player_busted(self, all_moves):
        ...
        if all_moves == ['y', 'n']:
        	return False
        if len(all_moves) == 0:
            # print("no avaliable action, busted")
    ...
    def check_value_availability(self, value):
        ...
        if self.count_neutral_markers() < 3:
            # so at most 3 markers for ine player
            return True
